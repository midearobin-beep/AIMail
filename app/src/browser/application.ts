...
export default class Application extends EventEmitter {
  ...
  async start(options) {
    ...
    try {
      ...
      if (err.toString().includes('ENOENT')) {
        message = localized(
          `AIMail could not find the mailsync process. If you're building AIMail from source, make sure mailsync.tar.gz has been downloaded and unpacked in your working copy.`
        );
      } else if (err.toString().includes('spawn')) {
        message = localized(`AIMail could not spawn the mailsync process. %@`, err.toString());
      } else {
        message = localized(
          `We encountered a problem with your local email database. %@\n\nCheck that no other copies of AIMail are running and click Rebuild to reset your local cache.`,
          err.toString()
        );
  ...
  async oneTimeAddToDock() {
    ...
    if (!addedToDock && appPath.includes('/Applications/') && appPath.includes('.app/')) {
      ...
      proc.exec(
        `defaults write com.apple.dock persistent-apps -array-add "<dict><key>tile-type</key><string>file-tile</string><key>tile-data</key><dict><key>file-type</key><integer>41</integer><key>file-label</key><string>AIMail</string><key>bundle-identifier</key><string>com.aimail.desktop</string><key>file-data</key><dict><key>_CFURLString</key><string>file://${appBundlePath}.app/</string><key>_CFURLStringType</key><integer>15</integer></dict></dict></dict>" && pkill "Dock"`
      );
  ...
  ensureWindowsForTokenState(behavior?: { preserveHiddenOrMinimized: boolean }) {
    ...
    if (hasAccount) {
      ...
      const title = localized('Welcome to AIMail');
